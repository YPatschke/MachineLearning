...
# Logistic model run
def runLogistic(user, input, output, learning_rate, batch_size, display_step, num_epoch, display_epoch, num_input, num_classes, train_size, timestep, path):
    tf.reset_default_graph()
    # split data
    train_input, train_output, test_input, test_output, num_examples = sp.split(input, output, train_size, False)

    # set tensorflow Graph Input
    ...
    # construct a simple linear model with the softmax function
    pred = tf.nn.softmax(tf.matmul(X, W) + b)

    # cost function that we need to minimize using cross entropy
