...
# Random Choice Probability Generator
def runRandomChoiceProbabilityGenerator(model, stepToGenerate, userModel, user_to_test, input, output, n_hiddens, n_neurons, num_input, num_classes, train_size, timestep, path):
    ...
    with tf.Session() as sess:
        ...
        for p in range(stepToGenerate):
            ...
            next_choice=int(np.random.choice(238, 1, p=np.array(pred_POIS[0])))
            print("New position - Random Choice Probability:"+str(next_choice))
            generate_path_choice.append(next_choice)
            print("="*150)

            POIS2=[0 for _ in range(238)]
            POIS2[[next_choice][0]]=1
        ...
        print("Path generated - Choice: "+ str(generate_path_choice))

        fig = plt.figure(facecolor='white')
        ax = fig.add_subplot(111)
        plt.plot(generate_path_choice, label='Random Choice Probability')
        plt.plot(TrueMovement[timestep:stepToGenerate+timestep], label='Real')
        plt.legend()
        plt.axis([0, len(generate_path_choice), 0, 240])
        plt.title("Generated trajectory - Random Choice Probability vs Real \n Movement of user: "+str(user_to_test))
        plt.savefig(path+'/Generate/user'+str(user_to_test)+'Choice.png')
        #plt.show(fig)
        plt.close(fig)

    print("Generation Finished!")

    return generate_path_choice, TrueMovement[timestep:stepToGenerate+timestep]
